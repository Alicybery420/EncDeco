# -*- coding=utf-8 -*-
# Copyright © Ali Cybery (Ali Cybery)
# Recode ? Silahkan
...
about =     '''
                ...
                {}Version           : {}1.0 (beta)
                {}Special thanks to : {}King OF Dark Web Ali Cybery 
                {}Code name         : {}Tytyd:v
                {}Team              : {}Ali Cybery Hacking team
                {}E-mail            : {}loydrestize@gmail.com 
...
def chat():
	...
	os.system('xdg-open https://api.whatsapp.com/send?phone=0730659047&text=Assalamualaikum+Roy+Lu+Masih+Jual+Janda?')
kunci = '''
 	{} ▄▀▀▀▄{} *Ali Cybery!!
	{} █   █{} *Rule the world 
	███████         ▄▀▀▄
...
def menu():
	...
	menu()
        ...
        try:
             sp.check_output([
             'am', 'start',
             'https://api.whatsapp.com/send?phone=0730659047&text=Report : ' + chat + ''])
        except:
...
def unpyc():
    ...
    if ask == 'y' or ask == 'Y':
        ...
        menu()
def mzb():
    print 'Decompile ALI,CYBERY,Base64'
    a = raw_input('File : ' )
    b = open(a).read().replace('exec(', 'x = ').replace('))))',')))')
    note = 'DECOMPILED BY ALI CYBERY'
    c = open('mi.py', 'w')
    if 'marshal' in b:
